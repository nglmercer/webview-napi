...
pub fn render_pixels(
    ...
    let width_nz = NonZeroU32::new(width).ok_or_else(|| {
        napi::Error::new(napi::Status::GenericFailure, "Invalid width")
    })?;
    let height_nz = NonZeroU32::new(height).ok_or_else(|| {
        napi::Error::new(napi::Status::GenericFailure, "Invalid height")
    })?;
    ...
    if let Some(map) = storage.as_mut() {
        ...
        if needs_create {
            ...
            map.insert(window_id, CachedSurface {
        ...
        if let Some(entry) = map.get_mut(&window_id) {
            ...
            let surface_ptr = &mut *resources.surface as *mut softbuffer::Surface<&tao::window::Window, &tao::window::Window>;
